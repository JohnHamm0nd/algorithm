...
n = int(input('n 입력: '))
arr1 = list(map(int, input('첫번째 지도입력: ').split()))
arr2 = list(map(int, input('두번째 지도입력: ').split()))
new_arr1 = []
new_arr2 = []
for a in arr1:
    new_arr1.append(binary(a, n))
for b in arr2:
    new_arr2.append(binary(b, n))
s_map = []
for i in range(len(new_arr1)):
    s_code = ''
    for j in range(len(new_arr1[i])):
        if int(new_arr1[i][j]) + int(new_arr2[i][j]) >= 1:
            s_code += '#'
        else:
            s_code += ' '
    s_map.append(s_code)
