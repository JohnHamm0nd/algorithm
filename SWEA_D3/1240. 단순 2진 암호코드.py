...
sys.stdin = open('1240.txt', 'r', encoding = 'UTF-8')

password_code = ['0001101', '0011001', '0010011', '0111101', '0100011', '0110001', '0101111', '0111011', '0110111', '0001011']


for T in range(int(input())):
    ver, hor = list(map(int, input().split()))
    flag = True
    for i in range(ver):
        password = input()
        if '1' in password and flag:
            flag = False
            arr = []
            p_index = password.rfind('1')
            for j in range(8, 0, -1):
                arr.append(password[p_index-7*j+1:p_index-7*(j-1)+1])
            brr = []
            for a in arr:
                brr.append(password_code.index(a))
            if ((brr[0] + brr[2] + brr[4]+ brr[6]) * 3 + brr[1] + brr[3] + brr[5] + brr[7]) % 10 == 0:
                print('#%d %d' %(T+1, sum(brr)))
            else:
                print('#%d %d' %(T+1, 0))
