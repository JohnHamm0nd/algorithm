...
sys.stdin = open('5431.txt', 'r', encoding = 'UTF-8')


for T in range(int(input())):
    students, submissions = list(map(int, input().split()))
    submission_number = list(map(int, input().split()))
    students_list = []
    for i in range(1, students+1):
        students_list.append(str(i))
    for s in submission_number:
        students_list.remove(str(s))
    print('#%d %s' %(T+1, ' '.join(students_list)))
